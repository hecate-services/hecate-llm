...
%% @doc Procedure name as advertised on the mesh. The hecate-services
%% convention is `<service>.<method>'; ?MODULE registers itself
%% under this name at boot.
...
%%
%% Uses macula:advertise_stream/2 (server_stream mode). Degrades
%% gracefully when no pool / no realm — registers nothing, but
%% the gen_server stays alive so test harnesses + the
